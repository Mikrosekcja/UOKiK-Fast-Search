...
(function() {
  var YAML, async, base_uri, data_dir, delay, fs, iconv, jsdom, links, request, save_term, scrape_list_page, url;
        ...
        scripts: ["https://raw.github.com/nbubna/HTML/0.9.4/dist/HTML.all.min.js"],
          ...
          if (cells.length !== 0) {
            cells.each(function(td) {
              ...
              }).slice(3).join("\n".trim().replace(/[ \t]+/, " ").replace(/^"/, '').replace(/\(.*?\)$/), ''.trim().replace(/"$/, ''));
              console.log(content);
              data = {
                meta: {
                  number: number,
